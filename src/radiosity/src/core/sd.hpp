  ...
  bool subdivide( MeshType& _m, size_t _n , const bool _update_points = true)
  {
    ...
    while ( !stop && ! queue.empty() ) {
      ...
      if ( a.second < max_edge_length_squared_ ) {
        ...
        {
            typename MeshType::HalfedgeHandle h0 = _m.halfedge_handle(a.first, 0);
            typename MeshType::HalfedgeHandle o0 = _m.halfedge_handle(a.first, 1);

            typename MeshType::HalfedgeHandle h1 = _m.next_halfedge_handle(h0);
            typename MeshType::HalfedgeHandle h2 = _m.next_halfedge_handle(h1);

            typename HalfedgeHandle o1 = _m.next_halfedge_handle(o0);
            typename HalfedgeHandle o2 = _m.next_halfedge_handle(o1);

            _m.split(a.first,newVertex);

            if (!_m.is_boundary(h0))
            {
