...
__constant__ float kernelFilter[FILTER_SIZE];
__constant__ int indexOffsetsU[25];
__constant__ int indexOffsetsV[25];
__constant__ float invScale;
__constant__ float offset;
...
__global__ void PostprocessKernel( uchar4* dst, unsigned int imgWidth, unsigned int imgHeight )
{
    ...
    {
#if USE_SHARED_MEM
        uchar4 color = sTex[sU + indexOffsetsU[i]][sV + indexOffsetsV[i]]; 
#else
    ...
    dst[index] = make_uchar4( Clamp<unsigned char>(tempColor.x * invScale + offset, 0.0f, 255.0f), Clamp<unsigned char>(tempColor.y * invScale + offset, 0.0f, 255.0f), Clamp<unsigned char>(tempColor.z * invScale + offset, 0.0f, 255.0f), 1 );
}
...
void PostprocessCUDA( cudaGraphicsResource_t& dst, cudaGraphicsResource_t& src, unsigned int width, unsigned int height, float* filter, float scale, float offset )
{
    ...
    cutilSafeCall( cudaMemcpyToSymbol("invScale", &invScale, sizeof(float), 0, cudaMemcpyHostToDevice ) );
    cutilSafeCall( cudaMemcpyToSymbol("offset", &offset, sizeof(float), 0, cudaMemcpyHostToDevice ) );
    
    // Copy the data in the filter to the constant device variable.
    cutilSafeCall( cudaMemcpyToSymbol("kernelFilter", filter, FILTER_SIZE * sizeof(float), 0, cudaMemcpyHostToDevice ) );

    // Copy the index offset arrays to constant memory 
    cutilSafeCall( cudaMemcpyToSymbol("indexOffsetsU", indexOffsetsU, 25 * sizeof(int), 0, cudaMemcpyHostToDevice) );
    cutilSafeCall( cudaMemcpyToSymbol("indexOffsetsV", indexOffsetsV, 25 * sizeof(int), 0, cudaMemcpyHostToDevice) );
